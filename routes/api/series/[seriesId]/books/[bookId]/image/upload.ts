...
export const handler: Handlers = {
  async POST(req, ctx) {
    ...
    const book = await kv.get<Book>(bookKey(user.id, seriesId, bookId));
    ...
    const objectKey =
      `yawt/user/${user.id}/series/${seriesId}/books/${bookId}/cover/${crypto.randomUUID()}`;
    ...
    try {
      ...
      console.error("Failed to upload book cover image to R2", {
        error: err,
        userId: user.id,
        seriesId,
        bookId,
