...
export const handler: Handlers = {
  async POST(req, ctx) {
    ...
    const location = await kv.get<Location>(
      locationKey(user.id, seriesId, locationId),
    );
    if (!location.value) return notFound("Location not found");
    ...
    const objectKey =
      `yawt/user/${user.id}/series/${seriesId}/locations/${locationId}/${crypto.randomUUID()}`;
