...
curl -s "${BASE_URL}/api/me" -c cookies.txt -b cookies.txt | jq 2>/dev/null || curl -s "${BASE_URL}/api/me" -c cookies.txt -b cookies.txt
echo -e "\n"

# Test creating a note
echo "2. Testing POST /api/notes (Create a note)"
NOTE_ID=$(curl -s -X POST "${BASE_URL}/api/notes" \
  -H "Content-Type: application/json" \
  -d '{"title": "Test Note", "content": "This is a test note created via API"}' \
  -b cookies.txt -c cookies.txt | jq -r '.note.id' 2>/dev/null)
echo "Created note with ID: ${NOTE_ID}"
echo -e "\n"

# Test listing notes
echo "3. Testing GET /api/notes (List all notes)"
curl -s "${BASE_URL}/api/notes" -b cookies.txt -c cookies.txt | jq 2>/dev/null || curl -s "${BASE_URL}/api/notes" -b cookies.txt -c cookies.txt
echo -e "\n"

# Test getting a specific note
if [ ! -z "$NOTE_ID" ] && [ "$NOTE_ID" != "null" ]; then
  echo "4. Testing GET /api/notes/${NOTE_ID} (Get specific note)"
  curl -s "${BASE_URL}/api/notes/${NOTE_ID}" -b cookies.txt -c cookies.txt | jq 2>/dev/null || curl -s "${BASE_URL}/api/notes/${NOTE_ID}" -b cookies.txt -c cookies.txt
  echo -e "\n"
  ...
  curl -s -X PUT "${BASE_URL}/api/notes/${NOTE_ID}" \
    ...
    -b cookies.txt -c cookies.txt | jq 2>/dev/null || curl -s -X PUT "${BASE_URL}/api/notes/${NOTE_ID}" -H "Content-Type: application/json" -d '{"title": "Updated Test Note", "content": "This note has been updated"}' -b cookies.txt -c cookies.txt
  echo -e "\n"

  # Test deleting the note
  echo "6. Testing DELETE /api/notes/${NOTE_ID} (Delete note)"
  curl -s -X DELETE "${BASE_URL}/api/notes/${NOTE_ID}" -b cookies.txt -c cookies.txt | jq 2>/dev/null || curl -s -X DELETE "${BASE_URL}/api/notes/${NOTE_ID}" -b cookies.txt -c cookies.txt
  echo -e "\n"
fi
